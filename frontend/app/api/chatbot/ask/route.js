...
function isUnsafeSql(sql) {
  ...
  return forbidden.some(keyword => new RegExp(`\\b${keyword}\\b`, 'i').test(text))
}
...
export async function POST(req) {
  ...
  if (!vannaBase) {
    return NextResponse.json({ error: 'VANNA_API_BASE is not configured' }, { status: 500 })
  }
  ...
  if (session.user.role !== 'admin') {
    const headData = await gqlFetch(GET_DEPARTMENT_BY_HEAD, { head_user_id: session.user.id })
    department = headData?.departments?.[0]

    if (!department) {
      const mentorData = await gqlFetch(GET_DEPARTMENT_BY_MENTOR, { mentor_id: session.user.id })
      ...
      if (departments.length === 1) {
        ...
        return NextResponse.json({ error: 'Multiple departments detected for mentor' }, { status: 403 })
      }
    }
    if (!department) {
      return NextResponse.json({ error: 'Department scope missing' }, { status: 403 })
    }
  ...
  try {
    ...
    if (!vannaResponse.ok) {
      ...
      return NextResponse.json({ error: errorText || 'Vanna error' }, { status: 502 })
    }

    const payload = await vannaResponse.json()
    const sql = payload?.sql || ''

    if (!sql || !sql.trim().toLowerCase().startsWith('select') || isUnsafeSql(sql)) {
      return NextResponse.json({ error: 'Unsafe SQL generated' }, { status: 400 })
    }

    if (department && !hasDepartmentFilter(sql, department.id)) {
      return NextResponse.json({ error: 'Department scope not enforced in SQL' }, { status: 403 })
    }
